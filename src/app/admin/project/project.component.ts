...
export class ProjectComponent implements OnInit {
    ...
    // parseHierarchy(data: CompanyHierarchyViewDto[], level: number, maxlevel: number, currentNode: VizModel) {
    //     if (level == 0) {
    //         let root = data.filter(value => value.hierarchyLevel === level)[0];
    //         currentNode.id = root.chartId;
    //         currentNode.level = root.hierarchyLevel;
    //         currentNode.name = root.roleName;
    //         data.filter(value => value.selfId === root.chartId).forEach(value => {
    //             if (!currentNode.children) currentNode.children = []
    //             currentNode.children.push({ name: value.roleName, level: value.hierarchyLevel, id: value.chartId })
    //         });
    //         currentNode.children.forEach((value) => {
    //             this.parseHierarchy(data, level + 1, maxlevel, value)
    //         })
    //     } else if (level <= maxlevel) {

    //         data.filter(value => value.selfId === currentNode.id).forEach(value => {
    //             if (!currentNode.children) currentNode.children = []
    //             currentNode.children.push({ name: value.roleName, level: value.hierarchyLevel, id: value.chartId })
    //         });
